...
>**内存与CPU传输数据的过程中容易受到电磁干扰导致发生比特翻转，使原来是0的数据
>变成1，原来是1的数据变成0
>![[电磁干扰.png]]
...
----
#### 奇偶校验
>**为了解决这个问题，人们提出了奇偶校验。在计算机中使用的是二进制数，我们可以在数据的开头增加一位纠错码
>![[Pasted image 20220717195801.png]]
>![[Pasted image 20220717195823.png]]

>**统计数据中1的个数，如果1的个数为偶数，那么纠错码填充为0
...
>
>>奇偶校验不是用一个数去校验其他数据，而是用一个数据改变整组数据的奇偶性，纠错码本身就处于该组数据中





>**但是奇偶校验只能单纯的判断数据是否有错，并不能定位错误数据的位置，发现错误后需要等待发送方重传数据，这样无形间会造成一定的延迟
...
#### 汉明码
>**为了解决以上两个问题，美国数学家图灵奖获得者理查德汉明在奇偶检验的基础上提出了汉明码，也就是目前大多数ecc内存使用的纠错码，它能用很少的纠错码对大量的数据进行错误查找和纠正
...
>那么为了定位错误数据位置，接下来就像找那只狗一样，需要有规律的将数据分成几组，并且让它们之间要有一定的交集，那么我们应该怎样进行分组？
...
>
>将2号位用作校验位，不再存放原始数据，而是对右半边的数据进行奇偶检验
...
>通过一定的排除和交集就能确定错误所在的行，结合1、2区域判断出来的列就能精确定位错误数据的位置
>![[Pasted image 20220717230221.png]]
>因为二进制数只能是1或0，一旦找到错误数据所在的位置重新翻转回来就可以实现纠正
...
>根据区域的检验结果可以定位错误位置在11号位，但是整个盘面有8个1是偶数，说明盘面通过了奇偶校验没有错误，这时盘面的结论和区域的结论发生了冲突，说明有两个错误，虽然没有办法确定两个错误的位置，但至少知道数据有错重新传输数据即可
...
>如果后期想要传输更多的数据，只要在数据中2的n次方位置放置一位纠错码就可以实现对更大块数据的支持。数据块越大，纠错码的占比越小，同时数据块中出现多个错误的概率也就越大
...
>因为汉明码需要额外的数据存放纠错码，所以普通8G内存只需要8颗1G的颗粒就可以实现8G可用容量，而ecc内存需要9颗1G颗粒来实现8G可用容量
>![[Pasted image 20220717235553.png]]

>**汉明码的优势不在于用少量的纠错码对大量的数据进行纠错
>![[Pasted image 20220717235858.png]]
>因为目前已经有更先进的LDPC纠错码，由香农的学生mit教授罗伯特在他的博士论文中提出，==5G上的编码用的就是这个==，目前广泛运用于SATA和NVME硬盘的纠错上，它能查找并解决多位比特翻转
...
>把原先十进制的位置下标换成二进制表示
>![[Pasted image 20220718014951.png]]
...
>这个时候如果有一位数据发生比特翻转变成1，直接把位置数穿插进来，相当于将其和0000进行异或，最后的结果就是错误的数据位一下就能找到错误
>![[Pasted image 20220718004049.png]]
>![[Pasted image 20220718004116.png]]
>翻转成0的结果也是一样的
>![[Pasted image 20220718004327.png]]

>**所以汉明码的纠错不需要像开头一样一个个区域排查演算，只要把1的位置数提取出来，将位置数按照顺序做异或运算，定位错误位置并纠正，所以汉明码的电路实现很简单，只要几个简单的逻辑门就可以实现，性能开销很小，开启ecc后的内存性能只会降低2%到3%左右，所以被广泛运用在内存纠错上。以上就是ecc内存的工作原理。
>![[Pasted image 20220718005147.png]]